<?php

return [

    'User created successfully' => 'تم إنشاء المستخدم بنجاح',
    'Invalid credentials' => 'بيانات تسجيل الدخول غير صحيحة',
    'User logged in successfully' => 'تم تسجيل الدخول بنجاح',
    'User logged out successfully' => 'تم تسجيل الخروج بنجاح',
    'Continue adding product to cart' => 'استمر في إضافة المنتج إلى السلة',
    'The product is not in the cart' => 'المنتج غير موجود في السلة',
    'The cart has been updated successfully' => 'تم تحديث السلة بنجاح',
    'Product removed' => 'تمت إزالة المنتج',
    'The basket has been emptied' => 'تم إفراغ السلة',
    'Category created successfully' => 'تم إنشاء الفئة بنجاح',
    'Category updated successfully' => 'تم تحديث الفئة بنجاح',
    'Category not found' => 'لم يتم العثور على الفئة',
    'Category deleted successfully' => 'تم حذف الفئة بنجاح',
    'Order created successfully' => 'تم إنشاء الطلب بنجاح',
    'Order status updated successfully' => 'تم تحديث حالة الطلب بنجاح',
    'Order deleted successfully' => 'تم حذف الطلب بنجاح',
    'product created successfully' => 'تم إنشاء المنتج بنجاح',
    'product updated successfully' => 'تم تحديث المنتج بنجاح',
    'product deleted successfully' => 'تم حذف المنتج بنجاح',
    "added to cart" => "تمت إضافة المنتج إلى السلة",
    "product not found" => "المنتج غير موجود في السلة",
        "cart updated" => "تم تحديث السلة",
        "removed from cart" => "تمت إزالة المنتج من السلة",
        "cart cleared" => "تم إفراغ السلة بالكامل"


];
